...
(function ($) {
	...
	$.fn.coinslider = $.fn.CoinSlider = function (options) {
		...
		var setFields = function (el) {
			var	tWidth	= parseInt(el.offsetWidth / params[el.id].spw),
				sWidth	= tWidth,
				tHeight	= parseInt(el.offsetHeight / params[el.id].sph),
				sHeight	= tHeight,
				counter	= 0,
				sLeft	= 0,
				sTop	= 0,
				i,
				j,
				tgapx	= el.offsetWidth - params[el.id].spw * sWidth,
				gapx	= tgapx,
				tgapy	= el.offsetHeight - params[el.id].sph * sHeight,
				gapy	= tgapy;
		...
		var init = function (el) {
			...
			options.images.forEach(c => {
				images[el.id].push(c[0])
				links[el.id].push(c[1]||"")
			...
			// create title bar
			$('#' + el.id).append("<div class='cs-title' id='cs-title-" + el.id + "' style='position: absolute; bottom:0; left: 0; z-index: 1000;right:0px'></div>");
